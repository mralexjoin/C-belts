...
vector<Spending> LoadFromJson(istream& input) {
  Document documet = Load(input);
  vector<Spending> spendings;
  for (const Node& node : documet.GetRoot().AsArray()) {
    const auto& spending = node.AsMap();
    spendings.push_back(
      {
        spending.at("category").AsString(),
        spending.at("amount").AsInt()
      }
    );
  }
  return spendings;
