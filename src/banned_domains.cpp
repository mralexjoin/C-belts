...
void TestSimple() {
  ...
  {
    Domain subdomain("m.ya.ru");
    Domain domain("ya.ru");
    ASSERT(IsSubdomain(subdomain, domain));
    ASSERT(!IsSubdomain(domain, subdomain));
  }
  {
    vector<Domain> banned_domains = {Domain("ya.ru"), Domain("m.ya.ru"), Domain("x.ya.ru")};
    DomainChecker domain_checker(banned_domains.begin(), banned_domains.end());
    ASSERT(domain_checker.IsSubdomain(Domain("t.ya.ru")));
  }
  {
    ASSERT_EQUAL(CheckDomains({Domain("ya.ru")}, {Domain("m.ya.ru")}), vector({false}));
  }
  {
    ostringstream output;
    vector<bool> results = {false, true};
    PrintCheckResults(results, output);
    ASSERT_EQUAL(output.str(), "Bad\nGood\n");
  }
  {
    istringstream input("1\nya.ru");
    vector<Domain> domains = ReadDomains(input);
    ASSERT_EQUAL(domains.size(), 1u);
    ASSERT_EQUAL(domains[0], Domain("ya.ru"));
  }
}
