...
HttpResponse& HttpResponse::SetContent(const string& a_content) {
  content = a_content;
  return *this;
}
...
void PrintHeader(ostream& output, const string& name, const string& value) {
  output << name << ": " << value << '\n';
}

ostream& operator<<(ostream& output, const HttpResponse& resp) {
  const string content_length_name = "Content-Length";
  output << "HTTP/1.1 " << resp.code << '\n';
  for (const auto& [name, value] : resp.headers) {
    if (name != content_length_name) {
      PrintHeader(output, name, value);
    }
  }
  if (!resp.content.empty()) {
    PrintHeader(output, content_length_name, to_string(resp.content.size()));
    output << '\n' << resp.content; 
  }
  else {
    output << '\n';
...
HttpResponse CommentServer::ServeRequest(const HttpRequest& req) {
  ...
  if (req.method == "POST") {
    if (req.path == "/add_user") {
      ...
      response.SetCode(HttpCode::Ok).SetContent(to_string(comments_.size() - 1));
      ...
      if (auto [id, input] = ParseIdAndContent(req.body); input == "42") {
    ...
    if (req.path == "/user_comments") {
      ...
      response.SetCode(HttpCode::Ok).SetContent(user_comments);
