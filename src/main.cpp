...
void TestMaxMemory(const Library& lib) {
  ...
  settings.max_memory = 50;
  auto cache = MakeCache(unpacker, settings);

  for (const auto& [name, book] : lib.content) {
    ASSERT_EQUAL(cache->GetBook(name).get(), cache->GetBook(name).get());
...
void TestCaching(const Library& lib) {
  ...
  // самостоятельно.
  auto p1 = cache->GetBook(lib.book_names[0]);
  cache->GetBook(lib.book_names[1]);
  auto p2 = cache->GetBook(lib.book_names[0]);
...
class MyUnpacker : public IBooksUnpacker {
public:
  unique_ptr<IBook> UnpackBook(const string& book_name) override {
    return make_unique<MyBook>(
      book_name,
      book_name
    );
  }
...
void MyTest() {
  // default_random_engine gen;
  // uniform_int_distribution<size_t> dis(0, lib.book_names.size() - 1);
  auto unpacker = make_shared<MyUnpacker>();
  ICache::Settings settings;
  settings.max_memory = 10;
  auto cache = MakeCache(unpacker, settings);
  auto ptr = cache->GetBook("0");
  for (size_t i = 0; i < 100; i++) {
    ASSERT_EQUAL(ptr, cache->GetBook("0"));
    cache->GetBook(to_string(i % 10));
  }
}
