...
public:
  ...
  AirportCounter() {
    departures_from_airports.fill(0);
  }
  ...
  size_t Get(TAirport airport) const {
    return departures_from_airports[CastToSizeT(airport)];
  }

  // добавить данный элемент
  void Insert(TAirport airport) {
    departures_from_airports[CastToSizeT(airport)]++;
  }
  ...
  void EraseAll(TAirport airport) {
    departures_from_airports[CastToSizeT(airport)] = 0;
  }
  ...
  Items GetItems() const {
    ...
    for (size_t i = 0; i < items.size(); i++) {
      items[i] = pair(CastToAirport(i),
                      departures_from_airports[i]);
    }
    return items;
  }

private:
  constexpr static TAirport CastToAirport(const size_t index) {
    return static_cast<TAirport>(index);
  }
  constexpr static size_t CastLastToSizeT() {
    return CastToSizeT(TAirport::Last_);
  }
  constexpr static size_t CastToSizeT(const TAirport airport) {
    return static_cast<size_t>(airport);
  }
  array<size_t, CastLastToSizeT()> departures_from_airports;
};
...
int main() {
  ...
  {
    LOG_DURATION("Test many get items");
