...
struct Address {
  ...
  bool operator==(const Address& other) const {
    return tuple(building, city, street) ==
           tuple(other.building, other.city, other.street);
  }
};
...
struct Person {
  ...
  bool operator==(const Person& other) const {
    return tuple(height, weight, name, address) ==
           tuple(other.height, other.weight, other.name, other.address);
  }
};

struct AddressHasher {
  size_t operator()(const Address& address) const {
    constexpr size_t FIRST_BASE = 659u;
    constexpr size_t SECOND_BASE = FIRST_BASE * FIRST_BASE;

    const hash<string> string_hasher;
    const hash<int> int_hasher;

    return string_hasher(address.city) * SECOND_BASE +
           string_hasher(address.street) * FIRST_BASE +
...
struct PersonHasher {
  size_t operator()(const Person& person) const {
    constexpr size_t FIRST_BASE = 1299721u;
    constexpr size_t SECOND_BASE = FIRST_BASE * FIRST_BASE;
    constexpr size_t THIRD_BASE = FIRST_BASE * SECOND_BASE;
    ...
    return string_hasher(person.name) * THIRD_BASE +
           int_hasher(person.height) * SECOND_BASE +
           double_hasher(person.weight) * FIRST_BASE +
